...
class UploadFile {//类定义开始
    ...
    private $config = array(
        ...
        'base64Data' => false, //是否为base64数据
        'base64Field' => '', //Base64的字段
        'isWater' => false, //是否添加水印
        'waterFile' => '', //水印图片地址
    );
    // 错误信息
    private $error = '';
    ...
    private function save($file) {
        ...
        if ($this->thumb && in_array(strtolower($file['extension']), array('gif', 'jpg', 'jpeg', 'bmp', 'png'))) {
            ...
            if (false !== $image) {
                ...
                if ($this->isWater) {
                    //添加水印
                    Image::water($filename, $this->waterFile);
                }
    ...
    private function base64ToImage($base64Field, $savePath) {
        ...
        $file['key'] = $base64Field;
        $file['extension'] = 'png';
        $file['savepath'] = $savePath;
        $file['savename'] = $this->getSaveName($file);
        $file['size'] = strlen($img);
        if (!file_put_contents($file['savepath'] . '/' . $file['savename'], $img)) {
            $this->error('输入输出错误！');
            return;
        }
        return $file;
    ...
    public function upload($savePath = '') {
        ...
        foreach ($files as $key => $file) {
            ...
            if (!empty($file['name'])) {
                ...
                if ($this->isWater) {
                    //添加水印
                    Image::water($file['savepath'] . $file['savename'], $this->waterFile);
                }
            }
        }
        if ($isUpload) {
